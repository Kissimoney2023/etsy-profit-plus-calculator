    ...
    <div className="py-24 bg-white dark:bg-transparent min-h-screen">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="mb-16">
          ...
          <h1 className="text-4xl md:text-6xl font-black font-heading text-secondary dark:text-white mb-8 leading-[1.1] tracking-tight">{post.title}</h1>
          <div className="flex flex-wrap items-center gap-6 text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-8">
            ...
            {post.author && <div className="flex items-center">By {post.author}</div>}
            <div className="flex items-center space-x-2">
              {post.keywords.map(kw => <span key={kw} className="bg-gray-50 dark:bg-slate-800 px-2 py-0.5 rounded border border-gray-100 dark:border-slate-700">#{kw}</span>)}
            </div>
          </div>
          {post.image && (
        ...
        <div className="prose prose-lg max-w-none prose-slate dark:prose-invert prose-headings:font-heading prose-headings:font-black prose-headings:text-secondary dark:prose-headings:text-white prose-a:text-primary prose-a:font-bold prose-strong:text-secondary dark:prose-strong:text-white prose-p:text-gray-500 dark:prose-p:text-slate-400 prose-p:leading-relaxed blog-content-styles">
          <div dangerouslySetInnerHTML={{ __html: post.content }} />
        </div>

        {/* Bottom CTA Box */}
        <div className="mt-20 p-10 bg-secondary dark:bg-slate-900 rounded-[40px] text-white relative overflow-hidden shadow-2xl">
